...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        private void ExecuteBuild ()
        {
            if (AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory)
            {
                ...
                string copyPath = m_streamingPath + "/AssetBundles/" + m_UserData.m_BuildTarget.ToString ();
                if (m_ForceRebuild.state)
                {
                    string message = "";
#if UNITY_IPHONE
                    message = "Do you want to delete all files in the directory" + m_UserData.m_OutputPath;
                    if (m_CopyToStreaming.state)
                        message += " and " + copyPath;
                    message += "?";
#else
                    message = "OutputPath:" + m_UserData.m_OutputPath;
                    if (m_CopyToStreaming.state)
                        message += "CopyPath:" + copyPath;
#endif
            ...
            PackageFile("Assets/Res/UI");
            PackageFile("Assets/Res/Module");
            //PackageSubDic("Assets/Res/Texture");

            //AssetPackages infos = XmlRes.CreatAssetPackages ("Assets/XmlConfig.xml");
            //List<string> sceneDir = new List<string> ();
            //foreach (var iter in infos.AssetList)
            //{
            //    switch (iter.PType)
            //    {
            //    case PackageType.PrefabAB:
            //        PackageFile (iter.Path);
            //        break;
            //    case PackageType.TextureAB:
            //        PackageSubDic (iter.Path);
            //        break;
            //    case PackageType.SceneAB:
            //        break;
            //    default:
            //        break;
            //    }
            ...
            if (m_CopyToStreaming.state)
            {
                Debug.Log (m_UserData.m_OutputPath + "-------------" + m_streamingPath);
                string copyPath = m_streamingPath + "/AssetBundles/" + m_UserData.m_BuildTarget.ToString ();
        ...
        /// <summary> ���Ŀ¼ Ŀ¼�ݹ�</summary>
        /// <param name="path"></param>
        static void PackageSubDic (string path)
        {
            Recursive (path, (filellist, name) =>
             {
                 foreach (var iter in filellist)
                 {
                     string tempName = iter.Replace ('/', '.').ToLower ();//���ļ��д��
                                                                          //string tempName = name.Replace('/', '.').ToLower();//��ͼƬ���
                     string bundleName = tempName.Replace ("assets.ar.res.", "");
                     ModifyAssetsVar (iter, bundleName);
                 }
             });
            AssetDatabase.SaveAssets ();
            AssetDatabase.Refresh ();
        ...
        /// <summary> ����ļ� һ��ab </summary>
        /// <param name="path"></param>
        static void PackageFile (string path)
        {
            Recursive (path, (filellist, name) =>
             {
                 foreach (var iter in filellist)
                 {
                     string tempName = iter.Split('.')[0].Replace('/', '.').ToLower();
                     string bundleName = tempName.Replace("assets.ar.res.", "");
                     //Debug.LogError(bundleName + "   --------------"+ iter);
                     ModifyAssetsVar (iter, bundleName);
                 }
             });
            AssetDatabase.SaveAssets ();
        ...
        /// <summary>����Ŀ¼������Ŀ¼</summary>
        public static void Recursive (string path, Action<List<string>, string> cb)
        {
            ...
            {
                var temp = dir.Replace ('\\', '/');
                Recursive (temp, cb);
            }
        }


        static List<string> AssetsVarfiles = new List<string> ();
        static void ModifyAssetsVar (string file, string assetName)
        {
            AssetImporter assetImporter = AssetImporter.GetAtPath (file);
            if (assetImporter == null) return;
            assetImporter.assetBundleName = assetName;
            AssetsVarfiles.Add (file);
        }

        static void ResetAssetsVar ()
        {
            foreach (var iter in AssetsVarfiles)
            {
                AssetImporter assetImporter = AssetImporter.GetAtPath (iter);
                assetImporter.assetBundleName = null;
            }
            AssetDatabase.RemoveUnusedAssetBundleNames ();
            AssetsVarfiles.Clear ();
        }
    ...
    public class AssetPackageInfo
    {
        ...
        public AssetPackageInfo (string path, PackageType ptype)
        {
            Path = path;
            PType = ptype;
        }
    ...
    public class CubePos
    {
        ...
        public CubePos (string x, string y, string z)
        {
            PosX = x;
            PosY = y;
            PosZ = z;
    ...
    public class CubePosPackages
    {
        [XmlElement ("CubePos")]
        public List<CubePos> CubeList = new List<CubePos> ();
    }
    public class AssetPackages
    {
        [XmlElement ("���Ŀ¼")]
        public List<AssetPackageInfo> AssetList = new List<AssetPackageInfo> ();
    }

    public class XmlRes
    {
        public static AssetPackages CreatAssetPackages (string xmlPath)
        {
            XmlSerializer serializer = new XmlSerializer (typeof (AssetPackages));
            FileStream file = new FileStream (xmlPath, FileMode.Open, FileAccess.Read);
            AssetPackages info = (AssetPackages)serializer.Deserialize (file);
            file.Close ();
            return info;
        }
    }
